...
window.ZentridLocalStore = window.ZentridLocalStore || (() => {
  ...
  function normalizePlantForClientModel(p: ZentridStoreRecord = {}): ZentridStoreRecord {
    return {
      ...
      address: p.address || '—',
      timezone: p.timezone || '—',
      capacityDc: typeof p.capacityDc === 'number' ? `${p.capacityDc} MWp` : (p.capacityDc || '—'),
      capacityAc: typeof p.capacityAc === 'number' ? `${p.capacityAc} MW` : (p.capacityAc || '—'),
      gridCapacity: typeof p.gridCapacity === 'number' ? `${p.gridCapacity} MW` : (p.gridCapacity || '—'),
      ...
      powerNow: p.powerNow || p.livePower || '—',
      energyToday: p.energyToday || p.today || '—',
