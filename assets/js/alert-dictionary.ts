...
function renderPage(): void { FleetLayout.mount(`<section class="page-hero"><div><p class="eyebrow">Global Admin · Data Governance</p><h1>Alert Dictionary</h1><p class="muted">Zentrid canonical alert dictionary and source vendor mappings controlled by the normalization layer.</p></div><button class="freshness-card" onclick="FleetLayout.toast('Alert dictionary is loaded')"><span class="pulse"></span><div><strong>v1.0.0</strong><small>28 alerts · 844 mappings</small></div></button></section>${shell(activeContent())}<aside class="detail-drawer alert-code-drawer-v95" id="alertCodeDrawer"><button class="drawer-close" type="button" id="alertCodeDrawerClose">x</button><div id="alertCodeDrawerBody"></div></aside>`); wire(); }
function wire(): void {
...
FleetLayout.mount('<section class="glass-card"><p class="muted">Loading Zentrid alert dictionary...</p></section>');
loadAlertDictionary();
