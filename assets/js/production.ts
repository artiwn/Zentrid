...
function refreshProduction(): void{ const host=document.getElementById('productionContent'); const bc=document.querySelector('.production-breadcrumb'); if(bc)bc.outerHTML=breadcrumbHtml(); if(host)host.innerHTML=renderProductionContent(); FleetLayout.enhanceActionMenus?.(document); }
