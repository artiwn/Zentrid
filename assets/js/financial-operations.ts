window.FinancialOperations = (() => {
  ...
  function clone(v: FleetLegacyCompat){ return JSON.parse(JSON.stringify(v)); }
  function safeParse(key: FleetLegacyCompat, fallback: FleetLegacyCompat){ try { return JSON.parse(localStorage.getItem(key) || JSON.stringify(fallback)); } catch(e){ return fallback; } }
  function savedPlans(){ return safeParse(tariffStorageKey, []); }
  function savePlans(rows: FleetLegacyCompat){ localStorage.setItem(tariffStorageKey, JSON.stringify(rows)); }
  function assignments(){ return safeParse(assignmentStorageKey, []); }
  function saveAssignments(rows: FleetLegacyCompat){ localStorage.setItem(assignmentStorageKey, JSON.stringify(rows)); }
  ...
  function esc(v: FleetLegacyCompat){ return String(v ?? '').replace(/[&<>"']/g, (c: FleetLegacyCompat) => (({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#039;' } as FleetLegacyCompat)[c])); }
  function n(v: FleetLegacyCompat){ const x = Number(v); return Number.isFinite(x) ? x : 0; }
  function money(v: FleetLegacyCompat, currency: FleetLegacyCompat = state.draft.currency){ const s = currencies[currency] || currency; const x = n(v); return currency === 'AMD' ? `${s}${Math.round(x).toLocaleString('en-US')}` : `${s}${x.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}`; }
  function badgeClass(v: FleetLegacyCompat){ const s=String(v).toLowerCase(); if(/paid|active|ready|approved|matched|completed/.test(s)) return 'success'; if(/draft|pending|review|processing|sent/.test(s)) return 'warning'; if(/overdue|failed|blocked|exception|disputed/.test(s)) return 'danger'; return 'info'; }
  function setByPath(path: FleetLegacyCompat, value: FleetLegacyCompat){
  ...
  function input(label: FleetLegacyCompat, path: FleetLegacyCompat, type: FleetLegacyCompat = 'number', step: FleetLegacyCompat = '1'){
    const val = path.split('.').reduce((o: FleetLegacyCompat, p: FleetLegacyCompat) =>o?.[p], state.draft);
  ...
  function linkedUsageInput(label: FleetLegacyCompat, path: FleetLegacyCompat, limitPath: FleetLegacyCompat, priceLabel: FleetLegacyCompat, pricePath: FleetLegacyCompat){
    const val = path.split('.').reduce((o: FleetLegacyCompat, p: FleetLegacyCompat) =>o?.[p], state.draft);
    const limit = limitPath.split('.').reduce((o: FleetLegacyCompat, p: FleetLegacyCompat) =>o?.[p], state.draft);
    const price = pricePath.split('.').reduce((o: FleetLegacyCompat, p: FleetLegacyCompat) =>o?.[p], state.draft);
  ...
  function select(label: FleetLegacyCompat, path: FleetLegacyCompat, opts: FleetLegacyCompat){ const val = path.split('.').reduce((o: FleetLegacyCompat, p: FleetLegacyCompat) =>o?.[p], state.draft); return `<label class="fo-field"><span>${esc(label)}</span><select data-fo-path="${esc(path)}" onchange="FinancialOperations.setField('${path}', this.value)">${opts.map((o: FleetLegacyCompat) =>`<option ${o===val?'selected':''}>${esc(o)}</option>`).join('')}</select></label>`; }
  function table(headers: FleetLegacyCompat, rows: FleetLegacyCompat, cls: FleetLegacyCompat = ''){ const cols = headers.length; return `<div class="data-table fo-data-table ${cls}" style="--fo-cols:${cols}"><div class="data-head">${headers.map((h: FleetLegacyCompat) =>`<span>${esc(h)}</span>`).join('')}</div>${rows.join('')}</div>`; }

  function tierCharge(qty: FleetLegacyCompat, tiers: FleetLegacyCompat, model: FleetLegacyCompat){
    qty = n(qty); if(qty <= 0 || !tiers.length) return 0;
    const clean = tiers.map((t: FleetLegacyCompat) =>({from:n(t.from||1), to:t.to===''||t.to==null?Infinity:n(t.to), price:n(t.price)})).sort((a: FleetLegacyCompat, b: FleetLegacyCompat) =>a.from-b.from);
    if(model === 'Flat') return qty * (clean[0]?.price || 0);
    if(model === 'Volume' || model === 'Tiered') { const tier = clean.find((t: FleetLegacyCompat) =>qty>=t.from && qty<=t.to) || clean[clean.length-1]; return qty * (tier?.price || 0); }
    if(model === 'Capacity-Based') return qty * (clean[0]?.price || 0);
    if(model === 'Revenue-Based') return qty * (clean[0]?.price || 0);
    return clean.reduce((sum: FleetLegacyCompat, t: FleetLegacyCompat) =>{ const upper = Math.min(qty,t.to); if(upper < t.from) return sum; return sum + ((upper - t.from + 1) * t.price); },0);
  }
  ...
  function calc(){
    ...
    const deviceLines = d.devices.map((dev: FleetLegacyCompat) => ({ ...dev, amount: tierCharge(dev.qty, dev.tiers, dev.pricingModel) }));
    const devices = deviceLines.reduce((s: FleetLegacyCompat, x: FleetLegacyCompat) =>s+x.amount,0);
    const modules = d.modules.filter((m: FleetLegacyCompat) =>m.enabled).reduce((s: FleetLegacyCompat, m: FleetLegacyCompat) =>s + (m.basis === 'Per MW' ? n(u.mw)*n(m.price) : n(m.price)),0);
  ...
  function devicePricingEditor(){
    ...
    return `<section class="panel fo-builder-panel fo-wide-panel">
      ...
      <div class="fo-device-list">
        ${d.devices.map((dev: FleetLegacyCompat, di: FleetLegacyCompat) =>`
          <article class="fo-device-card">
            ...
            <div class="fo-form-grid compact">
              <label class="fo-field"><span>Device type</span><select onchange="FinancialOperations.updateDevice(${di}, 'type', this.value)">${deviceTypes.map((x: FleetLegacyCompat) =>`<option ${x===dev.type?'selected':''}>${esc(x)}</option>`).join('')}</select></label>
              <label class="fo-field"><span>Vendor</span><select onchange="FinancialOperations.updateDevice(${di}, 'vendor', this.value)">${vendors.map((x: FleetLegacyCompat) =>`<option ${x===dev.vendor?'selected':''}>${esc(x)}</option>`).join('')}</select></label>
              <label class="fo-field"><span>Pricing model</span><select onchange="FinancialOperations.updateDevice(${di}, 'pricingModel', this.value)">${pricingModels.map((x: FleetLegacyCompat) =>`<option ${x===dev.pricingModel?'selected':''}>${esc(x)}</option>`).join('')}</select></label>
            ...
            ${table(['From qty','To qty','Unit price','Actions'], dev.tiers.map((t: FleetLegacyCompat, ti: FleetLegacyCompat) =>`<div class="data-row"><div><input class="fo-mini-input" type="number" min="1" value="${esc(t.from)}" oninput="FinancialOperations.updateTier(${di},${ti},'from',this.value)"></div><div><input class="fo-mini-input" type="number" min="1" placeholder="∞" value="${esc(t.to)}" oninput="FinancialOperations.updateTier(${di},${ti},'to',this.value)"></div><div><input class="fo-mini-input" type="number" min="0" step="0.01" value="${esc(t.price)}" oninput="FinancialOperations.updateTier(${di},${ti},'price',this.value)"></div><div class="row-actions fo-inline-actions" data-no-kebab="true"><button class="secondary-action" type="button" onclick="FinancialOperations.addTier(${di})">+ Tier</button><button class="danger-action" type="button" onclick="FinancialOperations.removeTier(${di},${ti})">Delete</button></div></div>`),'fo-tier-table')}
          </article>`).join('')}
      </div>
    </section>`;
  }
  ...
  function calculationTrace(){
    ...
    return `${table(['Charge','Amount','Rule'], rows.map((r: FleetLegacyCompat) =>`<div class="data-row"><div><strong>${r[0]}</strong></div><div><strong>${r[1]}</strong></div><div><span>${r[2]}</span></div></div>`),'fo-calc-table')}
  ...
  function assignDrawer(){
    ...
    return `<aside class="detail-drawer fo-assign-drawer ${state.assignOpen ? 'open' : ''}" id="tariffAssignDrawer">
      ...
      <div class="fo-form-grid compact">
        <label class="fo-field"><span>Tariff Plan</span><select id="foAssignTariff">${[...plans, currentPlanSnapshot()].filter((p,i,a)=>a.findIndex((x: FleetLegacyCompat) =>x.code===p.code)===i).map((p: FleetLegacyCompat) =>`<option value="${esc(p.code)}" ${p.code===currentCode?'selected':''}>${esc(p.name)} · ${esc(p.code)}</option>`).join('')}</select></label>
        <label class="fo-field"><span>Tenant</span><select id="foAssignTenant">${rows.map((t: FleetLegacyCompat) =>`<option value="${esc(t.id)}">${esc(t.name)} · ${esc(t.code)}</option>`).join('')}</select></label>
  ...
  function persistCurrentDraft(){
    ...
    const rows = savedPlans();
    const idx = rows.findIndex((x: FleetLegacyCompat) => x.code === snapshot.code);
    if(idx >= 0) rows[idx] = snapshot; else rows.unshift(snapshot);
    savePlans(rows);
    return snapshot;
  }
  function saveTariff(){ persistCurrentDraft(); FleetLayout.toast('Tariff saved to localStorage'); refresh('tariffs'); }
  function loadTariff(code: FleetLegacyCompat){ const plan = savedPlans().find((x: FleetLegacyCompat) => x.code === code); if(!plan) return; state.draft = clone(plan); FleetLayout.toast('Tariff loaded'); refresh('tariffs'); }
  function openAssignTariff(code: FleetLegacyCompat){ state.assignCode = code || state.draft.code; if(!savedPlans().some((x: FleetLegacyCompat) =>x.code===state.assignCode)) persistCurrentDraft(); state.assignOpen = true; refresh('tariffs'); }
  ...
  function assignTariffToTenant(){
    ...
    if(!tenantId){ FleetLayout.toast('No tenant selected'); return; }
    const plan = savedPlans().find((x: FleetLegacyCompat) =>x.code===tariffCode) || persistCurrentDraft();
    const tenant = tenants().find((x: FleetLegacyCompat) =>x.id===tenantId);
    const rows = assignments().filter((x: FleetLegacyCompat) => !(x.tenantId === tenantId && x.status === 'Active'));
    rows.unshift({
      id: 'TBA-' + Date.now(),
      tenantId,
    ...
    FleetLayout.toast('Tariff assigned to tenant billing profile');
    refresh('tariffs');
  }
  function syncSimulatorToLimits(){ Object.assign(state.draft.usage, { users:n(state.draft.includedUsers), plants:n(state.draft.includedPlants), mw:n(state.draft.includedMw), apiCalls:n(state.draft.includedApi), commands:n(state.draft.includedCommands) }); FleetLayout.toast('Simulator usage synced to included limits'); refresh('tariffs'); }

  function refresh(page: FleetLegacyCompat){
    const root=document.getElementById('fo-page-root');
    if(!root || root.dataset.page !== 'tariffs') return;
    const active = document.activeElement as FleetLegacyCompat;
    ...
    if(activePath){
      const nextActive = root.querySelector(`[data-fo-path="${CSS.escape(activePath)}"]`) as FleetLegacyCompat;
      if(nextActive){
  ...
  function removeDevice(i: FleetLegacyCompat){ if(state.draft.devices.length>1) state.draft.devices.splice(i,1); refresh('tariffs'); }
  function updateDevice(i: FleetLegacyCompat, k: FleetLegacyCompat, v: FleetLegacyCompat){ state.draft.devices[i][k] = k==='qty'?n(v):v; refresh('tariffs'); }
  function addTier(i: FleetLegacyCompat){ state.draft.devices[i].tiers.push({from:1,to:'',price:0}); refresh('tariffs'); }
  function removeTier(i: FleetLegacyCompat, t: FleetLegacyCompat){ if(state.draft.devices[i].tiers.length>1) state.draft.devices[i].tiers.splice(t,1); refresh('tariffs'); }
  function updateTier(i: FleetLegacyCompat, t: FleetLegacyCompat, k: FleetLegacyCompat, v: FleetLegacyCompat){ state.draft.devices[i].tiers[t][k] = k==='to' && v==='' ? '' : n(v); refresh('tariffs'); }
  function toggleModule(i: FleetLegacyCompat){ state.draft.modules[i].enabled=!state.draft.modules[i].enabled; refresh('tariffs'); }
  function updateModule(i: FleetLegacyCompat, k: FleetLegacyCompat, v: FleetLegacyCompat){ state.draft.modules[i][k] = k==='price'?n(v):v; refresh('tariffs'); }
  ...
  function selectInvoice(id: FleetLegacyCompat){ state.selectedInvoice=id; const p=document.getElementById('invoiceDetailPanel'); if(p) p.innerHTML=invoiceDetail(); }
  function render(page: FleetLegacyCompat = 'tariffs'){ let content=tariffBuilder(); if(page==='billing') content=billingManagement(); if(page==='invoices') content=invoiceCenter(); if(page==='payments') content=paymentSettings(); if(page==='settlements') content=revenueSettlements(); return `<main id="fo-page-root" data-page="${page}">${content}</main>`; }
