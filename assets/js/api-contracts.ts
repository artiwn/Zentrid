...
(function () {
  ...
  const clients = createContract<ZentridClientDto>(CONTRACT_DEFINITIONS.clients, (row, _index, context) => {
    ...
    return {
      ...
      language: context.safeText(context.firstOf(row, ['preferences.language', 'identity.preferredLanguage', 'language'], 'English')),
      timezone: context.safeText(context.firstOf(row, ['preferences.timeZone', 'timezone', 'timeZone'], 'Asia/Yerevan')),
      temperature: context.safeText(context.firstOf(row, ['preferences.temperatureUnit', 'temperatureUnit'], '°C')),
      currency: context.safeText(context.firstOf(row, ['preferences.currency', 'currency'], 'AMD')),
      irradiation: context.safeText(context.firstOf(row, ['preferences.irradiationUnit', 'irradiationUnit'], 'kWh/m2')),
      ...
      assignmentRole: context.safeText(context.firstOf(row, ['portalAccount.role', 'identity.role', 'assignmentRole', 'role'], '—')),
