...
function plantDetailFreshnessResult(record: ZentridPlantRecord): ZentridRecordFreshnessResult {
  return window.ZentridDataFreshness?.evaluateRecord(record, 'plant') || { status:'unknown', label:'Unknown', tone:'neutral', ageMs:null, ageLabel:'Timestamp unavailable', timestamp:null, basis:'Last data', backendStatus:String(record.dataQualityStatus || ''), contradictsBackend:false };
}
function plantDetailFreshness(record: ZentridPlantRecord): string {
  if (plantDetailOrigin(record) === 'local') {
    return ZentridEntityDetailUX.freshness(record, 'plant', {
      timestampKeys:['updated','raw.updatedAt'],
      localPrefix:'Last local change:',
      localEmpty:'Local prototype record · no backend freshness',
      unavailableEmpty:'No local freshness available'
    });
  }
  const freshness=plantDetailFreshnessResult(record);
  const backend=freshness.backendStatus || String(record.dataQualityStatus || '—');
  return `${freshness.label} · ${freshness.basis} ${freshness.ageLabel}${backend && backend !== '—' ? ` · Backend: ${backend}` : ''}${freshness.contradictsBackend ? ' · mismatch' : ''}`;
}
...
function plantTelemetryState(record: ZentridPlantRecord): { kind: 'ready' | 'empty' | 'partial'; title: string; message: string } {
  ...
  const uiFreshness=plantDetailFreshnessResult(record);
  if (String(record.health || '').toLowerCase().includes('fault') || uiFreshness.status === 'stale' || uiFreshness.status === 'very-stale' || uiFreshness.status === 'unknown') return { kind:'partial', title:'Telemetry may be incomplete', message:`Operational values are present, but UI freshness is ${uiFreshness.label}. Review the last-data timestamp before relying on the trend.` };
