...
function overviewProgressBars(): string {
  if (!ZentridOverviewData.zentridHealth.length) return overviewEmpty('No device health KPI', 'The backend has not returned global device status counters.');
  const total = ZentridOverviewData.zentridHealth.reduce((sum, item) => sum + Number(item.value || 0), 0);
  return ZentridOverviewData.zentridHealth.map(item => {
    const value = Number(item.value || 0);
    const percentage = total > 0 ? (value / total) * 100 : 0;
    return `<button class="health-row drill" data-panel="fleet" data-title="${overviewEscape(item.label)}" data-route="devices"><div class="health-label"><span>${overviewEscape(item.label)}</span><strong>${value.toLocaleString()} · ${percentage.toFixed(1)}%</strong></div><div class="progress"><div style="width:${Math.max(0, Math.min(100, percentage))}%"></div></div></button>`;
  }).join('');
}
...
function overviewAlerts(): string {
  if (ZentridOverviewData.alertState === 'pending') return overviewEmpty('Loading alerts', 'Operational alert records are still loading in the background.');
  if (ZentridOverviewData.alertState === 'error' && !ZentridOverviewData.alerts.length) return overviewEmpty('Alerts unavailable', 'The alert endpoint did not complete. No placeholder records are shown.');
  if (!ZentridOverviewData.alerts.length) return overviewEmpty('No alert records', 'The alert API completed successfully and returned no records.');
...
function overviewIntegrations(): string {
  if (ZentridOverviewData.integrationState === 'pending') return overviewEmpty('Loading integration health', 'Operational integration summaries are still loading in the background.');
  if (ZentridOverviewData.integrationState === 'error' && !ZentridOverviewData.integrations.length) return overviewEmpty('Integration health unavailable', 'The integration summary endpoint did not complete. No zero-value placeholder is shown.');
  if (!ZentridOverviewData.integrations.length) return overviewEmpty('No integration records', 'The integration API completed successfully and returned no records.');
  return `<div class="table-list">${ZentridOverviewData.integrations.map(item => `<div class="table-row actionable"><button class="row-main drill" data-panel="integration" data-title="${overviewEscape(item.name)}" data-route="integrations"><strong>${overviewEscape(item.name)}</strong><small>Last sync: ${overviewEscape(item.sync)}</small></button><span class="badge ${overviewStatusClass(item.status)}">${overviewEscape(item.status)}</span><small>${overviewEscape(item.errors)}</small></div>`).join('')}</div>`;
}
...
function overviewRender(state: OverviewState = ZentridLayout.state): string {
  return `<section class="page-hero"><div><p class="eyebrow">Platform Operations Center · API-only</p><h1>Global Admin Overview</h1><p class="muted">This screen contains only values derived from backend responses. Prototype metrics, maps, trends and activity records were removed.</p></div><button class="freshness-card" type="button" data-live-refresh="overview"><span class="pulse"></span><div><strong>Data status</strong><small>Waiting for API response</small></div></button></section>
    ...
    ${overviewKpis()}
    <section class="dashboard-grid two-col"><article class="panel glass-card"><div class="panel-head"><div><h2>Device Health</h2><p>Global Online vs Attention counters returned by the device registry KPI.</p></div><button class="go" data-route="devices">Open Devices</button></div><div class="health-bars">${overviewProgressBars()}</div></article><article class="panel glass-card"><div class="panel-head"><div><h2>API Quality Summary</h2><p>Provider, template and stale-plant coverage from backend responses. Ambiguous backend health fields are shown raw and are not reinterpreted.</p></div><button class="go" data-route="api-console">API Console</button></div>${overviewQuality()}</article></section>
