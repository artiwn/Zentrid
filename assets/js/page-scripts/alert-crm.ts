...
const alertInboxV126: AlertCrmInboxItemV54[] = [
    ...
    function toast(msg: string): void { if(window.ZentridLayout && ZentridLayout.toast) ZentridLayout.toast(msg); else alert(msg); }
    function rowAction(label: string, js: string): string { return `<div class="row-actions alert-crm-actions-v126"><button class="small-btn" type="button" onclick="ZentridAlertCRMv126.${js}">${label}</button></div>`; }
    ...
    function actionPanel(): string {
      return `<div class="alert-crm-action-panel-v126">
        <button class="primary-action" type="button" onclick="ZentridAlertCRMv126.toast('Create case wizard opened')">Create Case</button>
        <button class="secondary-action" type="button" onclick="ZentridAlertCRMv126.toast('Task assignment panel opened')">Assign Task</button>
        <button class="secondary-action" type="button" onclick="ZentridAlertCRMv126.toast('SLA rules review opened')">Review SLA</button>
        <button class="secondary-action" type="button" onclick="ZentridAlertCRMv126.toast('Operations audit export prepared')">Export Activity</button>
      </div>`;
    }
    function overview(): string {
      return `<div class="section-title-v17 alert-crm-title-v126"><div><h2>Alert CRM Overview</h2><p class="muted">Turns technical alerts into managed cases, tasks, work orders, SLA controls and resolution history.</p></div><button class="primary-action" onclick="ZentridAlertCRMv126.toast('Create case wizard opened')">Create Case</button></div>
    ...
    window.ZentridAlertCRMv126 = { toast, open(type: string, title: string, meta: string): void { toast(type+': '+title+' — '+meta); } };
