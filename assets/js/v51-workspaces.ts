...
const V51: V51WorkspaceApi = (() => {
  ...
  function hero(eyebrow: string, title: string, desc: string, status = 'Updated now'): string { return `<section class="page-hero"><div><p class="eyebrow">${eyebrow}</p><h1>${title}</h1><p class="muted">${desc}</p></div><button class="freshness-card" onclick="ZentridLayout.toast('${title} refreshed')"><span class="pulse"></span><div><strong>${status}</strong><small>v5.1 enterprise workspace</small></div></button></section>`; }
