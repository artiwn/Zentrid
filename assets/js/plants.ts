interface ZentridPlant {
  ...
  registryPlantId?: string | number;
  canonicalPlantId?: string | number;
  operationalId?: string | number;
  operationalExternalId?: string | number;
...
function plantUiFreshness(p: ZentridPlant): ZentridRecordFreshnessResult {
  return window.ZentridDataFreshness?.evaluateRecord(p, 'plant') || { status:'unknown', label:'Unknown', tone:'neutral', ageMs:null, ageLabel:'Timestamp unavailable', timestamp:null, basis:'Last data', backendStatus:String(p.freshness || ''), contradictsBackend:false };
}
function plantFreshnessHtml(p: ZentridPlant): string {
  ...
  return `<span class="badge ${freshness.tone}" title="${plantDisplay(title)}">${plantDisplay(freshness.label)}</span><small>${plantDisplay(freshness.basis)} ${plantDisplay(freshness.ageLabel)} · Backend: ${plantDisplay(backend)}${freshness.contradictsBackend ? ' · mismatch' : ''}</small>`;
}
...
function plantOperationalId(plant: ZentridPlant | undefined): string {
  if (!plant) return '';
  const raw = plant.raw && typeof plant.raw === 'object' ? plant.raw : {};
  const operationalData = raw.operationalData && typeof raw.operationalData === 'object' ? raw.operationalData as Record<string, unknown> : {};
  const liveRecord = raw.liveRecord && typeof raw.liveRecord === 'object' ? raw.liveRecord as Record<string, unknown> : {};
  const candidate = plant.operationalId || plant.canonicalPlantId || operationalData.canonicalPlantId || liveRecord.id;
  return candidate === undefined || candidate === null ? '' : String(candidate).trim();
}
...
async function resolvePlantOperationalId(plant: ZentridPlant): Promise<string> {
  ...
  const adminId = plantAdministrativeId(plant) || String(plant.id || '').trim();
  if (!adminId || !window.ZentridPlatformAPI?.plantRegistry?.get) return '';
  try {
    const payload = await window.ZentridPlatformAPI.plantRegistry.get(adminId);
    const detail = payload && typeof payload === 'object' ? payload as Record<string, unknown> : {};
    const operationalData = detail.operationalData && typeof detail.operationalData === 'object' ? detail.operationalData as Record<string, unknown> : {};
    const canonicalId = String(operationalData.canonicalPlantId || detail.canonicalPlantId || '').trim();
    if (canonicalId) {
...
async function openPlantAlerts(plant: ZentridPlant): Promise<void> {
  ...
  if (!canonicalPlantId) {
    window.ZentridLayout?.toast?.('Operational Plant ID could not be resolved. Alerts were not opened with a Registry UUID.');
    return;
  }
...
function plantBackendDocuments(p: ZentridPlant): Array<Record<string, unknown>> {
  const raw = p.raw && typeof p.raw === 'object' ? p.raw : {};
  const adminRecord = raw.adminRecord && typeof raw.adminRecord === 'object' ? raw.adminRecord as Record<string, unknown> : {};
  const candidates = [p.documents, raw.documents, adminRecord.documents];
  ...
  return rows.filter(row => {
    const key = String(row.id || row.documentId || row.fileName || row.name || row.title || JSON.stringify(row)).trim();
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
}
function plantDocumentsPanel(p: ZentridPlant): string {
  const docs = plantBackendDocuments(p);
  if (!docs.length) return `<div class="empty-state"><strong>No plant documents returned</strong><small>The current Plant Registry detail contains no document records. Zentrid does not fabricate PPA, grid or technical passport files.</small></div>`;
  return `<div class="data-table compact-table plant-documents-v91"><div class="data-head"><span>Document</span><span>Type</span><span>Status</span><span>Updated</span></div>${docs.map(doc => {
    const name = doc.documentName || doc.fileName || doc.name || doc.title || doc.id || doc.documentId || 'Document';
    const type = doc.documentType || doc.type || doc.category || '—';
    const status = doc.status || doc.verificationStatus || 'Returned by API';
    const updated = doc.updatedAtUtc || doc.createdAtUtc || doc.uploadedAtUtc || doc.expiryDate || '—';
    return `<div class="data-row"><div><strong>${plantDisplay(name)}</strong><small>${plantDisplay(doc.fileName || doc.documentId || doc.id || 'Backend document')}</small></div><div><strong>${plantDisplay(type)}</strong><small>Plant Registry</small></div><div><span class="badge neutral">${plantDisplay(status)}</span></div><div><strong>${plantDisplay(updated)}</strong><small>Backend timestamp/value</small></div></div>`;
  }).join('')}</div>`;
}
