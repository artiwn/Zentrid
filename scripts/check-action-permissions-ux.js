...
[
  'const FleetActionPermissions', 'currentProfile', 'ZentridAuth?.getRoles',
  ...
  'Permissions are still being verified', 'does not allow', 'Object.assign(window, { FleetActionPermissions })'
...
[
  [tenants, 'data-permission-resource="tenant"', "FleetActionPermissions.guard({ action:'create', resource:'tenant' })"],
  [lifecycle, 'data-permission-action="${action}"', "FleetActionPermissions.guard({ action, resource:'tenant'"],
  [hierarchy, 'data-permission-resource="client"', "FleetActionPermissions.guard({ action:'create', resource:'client' })"],
  [hierarchy, 'data-permission-resource="plant"', "FleetActionPermissions.guard({ action:'edit', resource:'plant'"],
  [plants, 'data-permission-resource="plant"', "FleetActionPermissions.guard({ action:'create', resource:'plant' })"],
  [integrations, 'data-permission-resource="integration"', "FleetActionPermissions.guard({ action:'create', resource:'integration' })"]
