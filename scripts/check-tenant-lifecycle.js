...
const windowObject = {
  ZentridDataSource: {
    origin(record) { return String(record?.dataOrigin || 'unavailable'); }
  },
...
(async () => {
  if (lifecycle) {
    ...
    expect(!unavailableHtml.includes('data-tenant-lifecycle-action='), 'Unavailable tenant must not expose backend lifecycle buttons.');
  ...
  console.log(`Tenant lifecycle checks OK: live/unavailable gating, status actions, confirmation, mutation success and retry behavior verified.`);
})().catch(error => {
