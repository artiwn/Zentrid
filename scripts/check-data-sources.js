...
function runBehaviorChecks() {
  ...
  expect(helper.origin({ id: 'DEV-INV-00432' }, 'device') === 'mock', 'Seed records are not classified as mock.');
  expect(helper.markChanged({ dataOrigin: 'live', id: 'LIVE-1' }, 'device').dataOrigin === 'mixed', 'Changed live records must become mixed.');
  expect(helper.markChanged({ id: 'MOCK-1' }, 'device').dataOrigin === 'local', 'Changed mock records must become local.');
